...
def normalizar_tipo_propiedad(tipo: str) -> str:
    ...
    if any(k in t for k in ["apartoquinta", "aparto quinta", "apartoquita"]):
        return "apartoquinta"
...
def es_nombre_persona_valido(nombre: str) -> bool:
    ...
    palabras = [
        p for p in re.findall(r"[A-Za-zÀ-ÖØ-ÿ'´-]+", nombre)
        if normalizar_texto(p) not in STOPWORDS_NOMBRE_EXTRA and len(p) >= 2
    ]
...
def parsear_presupuesto_texto(texto: str):
    ...
    if (
        ...
        and not any(k in t for k in [
            "presupuesto", "maximo", "máximo", "hasta", "usd", "dolar",
            "dolares", "$", "mil", "millon", "millones", "k", "mm"
        ])
    ):
        return None
...
def necesita_actualizar_inventario(force: bool = False) -> bool:
    if force or not cache["inventario"]:
        return True
...
def sincronizar_google_sheet():
    ...
    try:
        ...
        pairs = (
            captadores_payload.items()
            if isinstance(captadores_payload, dict)
            else [
                (
                    r.get("nombre") or r.get("name"),
                    r.get("telefono") or r.get("phone")
                )
                for r in (
                    captadores_payload
                    if isinstance(captadores_payload, list)
                    else []
                )
...
def obtener_telefono_captador_de_sheet(nombre_captador: str) -> str:
    ...
    for nombre_sheet, telefono in sheets_cache["captadores"].items():
        ...
        if score > best_score:
            best_score, best_phone = score, telefono
...
def generar_respuesta_conversacional_paty(
    mensaje_usuario: str,
...
def fusionar_filtros(base: dict, nuevos: dict, mensaje: str) -> dict:
    ...
    if nuevos.get("tipo_operacion"):
        out["tipo_operacion"] = normalizar_operacion(nuevos["tipo_operacion"])
    ...
    if nuevos.get("presupuesto") not in [None, "", []]:
        try:
            out["presupuesto"] = float(nuevos["presupuesto"])
        except Exception:
            pass

    if any(k in txt for k in ["townhouse", "towhouse"]):
    ...
    if any(k in txt for k in ["comprar", "venta"]):
        out["tipo_operacion"] = "venta"
    elif any(k in txt for k in ["alquiler", "renta"]):
...
def fusionar_lead(base: dict, nuevos: dict, mensaje: str, sender: str) -> dict:
    ...
    if not out.get("nombre"):
        ...
        limpias = [
            p for p in palabras
            if normalizar_texto(p) not in STOPWORDS_NOMBRE_EXTRA and len(p) > 1
        ]
    ...
    if (
        not out.get("whatsapp")
        and len(limpiar_telefono(sender)) >= 7
        and limpiar_telefono(sender).isdigit()
    ):
...
def coincide_tipo_propiedad(propiedad: dict, tipo_filtro: str) -> bool:
    ...
    if tipo == "apartoquinta":
        return any(k in titulo for k in ["apartoquinta", "aparto quinta", "apartoquita"])

    if tipo == "casa":
        return any(
            k in titulo or k in tipo_wasi
            for k in ["casa", "townhouse", "apartoquinta", "aparto quinta", "apartoquita"]
        )
    ...
    if tipo == "apartamento":
        return any(k in titulo or k in tipo_wasi for k in ["apartamento", "penthouse"])
...
def elegir_top_n_propiedades(
    ...
    if tipo_prop:
        props = [
            p for p in props
            if coincide_tipo_propiedad(p, tipo_prop)
        ]
...
def formatear_ficha_propiedad(propiedad: dict, es_colega=False) -> str:
    ...
    precio_val = (
        propiedad.get("precio_renta_float", 0)
        if op == "alquiler"
        else propiedad.get("precio_venta_float", 0)
    )

    precio_str = (
        formato_moneda(precio_val)
        if precio_val > 0
        else propiedad.get("renta" if op == "alquiler" else "venta", "N/D")
    )
    ...
    if es_colega:
        ...
        lineas.extend([
            f"👤 Captador: {captador}",
            f"📲 WhatsApp Captador: {tel if tel != 'N/D' else propiedad.get('telefono_captador_wasi', 'N/D')}"
        ])
...
def construir_resumen_necesidad(filtros: dict) -> str:
    ...
    if filtros.get("tipo_propiedad"):
        p.append(f"- Tipo: {filtros['tipo_propiedad']}")

    if filtros.get("tipo_operacion"):
        p.append(f"- Operación: {filtros['tipo_operacion']}")

    if filtros.get("zona"):
        p.append(f"- Zona de interés: {filtros['zona']}")

    if filtros.get("presupuesto"):
        p.append(f"- Presupuesto máximo: {formato_moneda(filtros['presupuesto'])}")
...
def enviar_notificaciones_telegram(agente, lead: dict, resumen_necesidad: str):
    ...
    if agente and agente.get("telegram_id"):
        enviar(
            ...
            f"👤 ¡Tienes un nuevo lead asignado!\n\n{base_msg}"
        )
...
async def handle_request(request: Request):
    try:
        ...
        if request.headers.get("x-api-key") not in [
            k.strip()
            for k in os.getenv("API_KEYS_AGENTES", "").split(",")
            if k.strip()
        ]:
        ...
        async with lock:
            ...
            if (
                ...
                and (
                    ahora_ts - float(estado.get("ultimo_mensaje_ts", 0.0))
                ) <= VENTANA_MENSAJE_DUPLICADO_SEGUNDOS
            ):
                return {"replies": []}
            ...
            if not cache["inventario"]:
                ...
                asyncio.create_task(
                    garantizar_inventario_actualizado(force=False)
                )
            ...
            if analisis.get("rol") in {"cliente", "colega_inmobiliario"}:
            ...
            if not estado["rol"]:
                estado["rol"] = (
                    "colega_inmobiliario"
                    if any(
                        k in normalizar_texto(mensaje_cliente)
                        for k in [
                            "colega",
                            "broker",
                            "realtor",
            ...
            if (
                ...
                and not mensaje_parece_contacto(mensaje_cliente)
            ):
                estado["filtros"] = fusionar_filtros(
                    estado["filtros"],
                    analisis.get("filtros", {}),
                    mensaje_cliente
                )
            ...
            if estado["estado"] == ESTADO_DIAGNOSTICO_IA:
                ...
                if intencion in ["mas_opciones", "ajustar_busqueda"]:
                    ...
                    (
                        intencion in ["interes_propiedad", "enviar_datos"]
                        or "interesa" in normalizar_texto(mensaje_cliente)
                    )
            ...
            if (
                ...
                if not es_nombre_persona_valido(
                    estado["lead"].get("nombre", "")
                ):
                    faltante_lead = "Nombre Completo (Nombre y Apellido)"
                elif not estado["lead"].get("correo"):
                    faltante_lead = "Correo electrónico"
                elif not estado["lead"].get("whatsapp"):
                    faltante_lead = "Número de WhatsApp"
                ...
                if faltante_lead:
                    ...
                    enviar_notificaciones_telegram(
                        agente,
                        estado["lead"],
                        construir_resumen_necesidad(estado["filtros"])
                    )

                    contexto_sistema["agente_asignado"] = (
                        agente["nombre"]
                        if agente
                        else "un asesor experto"
                    )
            ...
            return {
                "replies": [
                    {
                        "message": respuesta_paty.replace("**", "*")
                    }
                ]
            }
        ...
        return {
            "replies": [
                {
                    "message": "Lo siento, estamos procesando tu solicitud... ¿Me repites por favor? 🙏"
                }
            ]
        }
