...
FRASES_HUMANO = [
    ...
    "comunicame con un agente",
    "comunicame con un asesor",
    "comunicame con alguien",
    "comunicame con soporte",
    "comunicame con un humano",
    "comunicame con una persona",
    "conectame con un agente",
    "conectame con un asesor",
    "conectame con alguien",
    "conectame con un humano",
    "conectame con una persona",
    "pasenme con un agente",
    "pasenme con un asesor",
    "paseme con un humano",
    "paseme con una persona",
    "transfierame con un asesor",
    "transfierame con un agente",
    "transfiereme con un humano",
    "transfiereme con una persona",
]
...
def obtener_ciudades_para_zona(zona: Optional[str]) -> Set[str]:
    ...
    for ciudad in ciudades_raw:
        ciudad_texto = str(ciudad or "").strip()
        clave_ciudad = normalizar_texto(ciudad_texto)
        if clave_ciudad:
            ciudades_por_normalizada.setdefault(clave_ciudad, ciudad_texto)
    ciudades = set(ciudades_por_normalizada.values())
    ...
    for zona_catalogo, ciudades_catalogo in catalogo_geografico["zonas_por_ciudad"].items():
        ...
        if (
            ...
            for ciudad in ciudades_catalogo:
                ciudad_texto = str(ciudad or "").strip()
                clave_ciudad = normalizar_texto(ciudad_texto)
                if clave_ciudad:
                    ciudades_por_normalizada.setdefault(clave_ciudad, ciudad_texto)
    ...
    if not ciudades:
        for ciudad in FALLBACK_ZONAS_AMBIGUAS.get(zona_norm, []):
            ciudad_texto = str(ciudad or "").strip()
            clave_ciudad = normalizar_texto(ciudad_texto)
            if clave_ciudad:
                ciudades_por_normalizada.setdefault(clave_ciudad, ciudad_texto)
